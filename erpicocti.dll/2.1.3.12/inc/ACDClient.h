...
#define CFG_CTI_AUTOLOGOUT "cti.windows.autologout"
#define CFG_CTI_AUTOPAUSE_ON_LOCK "cti.autopause.onlock"
...
private:
  ...
  SafeVar<CTime> m_cCallStartTime; 
  bool m_bAutoAnswer;
...
public:
  ...
  int RemoveFromQueues(const char* strUser, const char* strLocation);
public:
  void Initialize(Config *pConfig/* = NULL*/, const User *pUser/* = NULL*/, const char* strHost = NULL);
...
private:
  ...
  int CallToService(const char* strRequest, Json::Value* jsonResponse = 0, int callToWeb = 0);  
