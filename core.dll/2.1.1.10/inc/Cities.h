...
public:
  struct sCity
  {
    ...
    sCity& operator = (const sCity &another)
    {
      ...
      iTimezoneId                 = another.iTimezoneId;
  ...
  struct FindCity
  {
    ...
    FindCity(const int &_iCityId)
    {
      iCityId = _iCityId;
    }
  
    bool operator () (const CCities::sCity &_stCity) const
    {
      return (_stCity.dwId == iCityId);
    }
  };
  ...
  static bool GetServiceByPhone(const std::string &_strPhone, CCities::sCity &_stCity);
};
