...
inline std::string CorrectTimeForSQL(const DWORD &_dwTime)
{
  std::string strTimeValue;

  if (!_dwTime)
  {
      strTimeValue = "'0000-00-00 00:00:00'";
  }
  else
  {
      strTimeValue = "FROM_UNIXTIME(";
      strTimeValue += std::to_string(static_cast<long long>(_dwTime));
      strTimeValue += ")";
  }
...
public:
  ...
  enum eParams{
    ...
    BUS4US_ADDRESS_ONLY_NAME                   = 157,
    BTONE_DEFAULT_TIMEZONE                     = 158,
    BUS4US_TEXT_TO_CLIENT_20_MIN_AFTER_PAYMENT = 159,
    BUS4US_TEXT_TO_CLIENT_40_MIN_AFTER_PAYMENT = 160,
    BTONE_SEND_MAIL_TO_MANAGER                 = 161,
    BTONE_SEND_MAIL_TO_SERVICE                 = 162
  };
  ...
  static bool IsFileExists(const std::string &_strFileName);
};
