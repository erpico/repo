...
public:
    ...
    /* Проверка токена */
	bool CheckToken(const std::string &_strToken, Json::Value &root, Json::Value &result);
    ...
    /* Добавить отзыв по заказу */
    void GetOrderReview(Json::Value &root, Json::Value &result);
    ...
    /* Принять или отклонить заказа подрядчиком */
